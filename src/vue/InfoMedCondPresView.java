...
import controller.*;
import controllerFX.InfoMedCondPresController;
...
public class InfoMedCondPresView implements ViewMaker {
	private Stage stage;
	private AdminDAO dao;
	private final TableView<CIS_bdpm> table;
	private InfoMedCondPresController con;
	

	public InfoMedCondPresView(Stage stage) {
		this.stage = stage;
		table = new TableView<CIS_bdpm>();
		con= new InfoMedCondPresController(stage);
	}
	...
	public Scene getScene() {
		...
		Text title = new Text("Liste des conditions et prescription des médicaments ");
        ...
        Callback<TableColumn<CIS_bdpm, Void>, TableCell<CIS_bdpm, Void>> cellFactory = new Callback<TableColumn<CIS_bdpm, Void>, TableCell<CIS_bdpm, Void>>() {
            ...
            public TableCell<CIS_bdpm, Void> call(final TableColumn<CIS_bdpm, Void> param) {
                final TableCell<CIS_bdpm, Void> cell = new TableCell<CIS_bdpm, Void>() {
                    ...
                    {
                    	conBtn.setOnMousePressed(e -> con.redirectcond(e,  getTableView().getItems().get(getIndex()).getCode_cis()));
                    }
