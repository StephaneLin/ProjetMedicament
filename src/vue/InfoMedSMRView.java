...
import controller.*;
import controllerFX.InfoMedSMRController;
...
public class InfoMedSMRView implements ViewMaker {
	private Stage stage;
	private AdminDAO dao;
	private final TableView<CIS_bdpm> table;
	private InfoMedSMRController con;
	//private int CIS;

	public InfoMedSMRView(Stage stage) {
		this.stage = stage;
		//this.CIS=cis;
		table = new TableView<CIS_bdpm>();
		con= new InfoMedSMRController(stage);
	}
        ...
        Callback<TableColumn<CIS_bdpm, Void>, TableCell<CIS_bdpm, Void>> cellFactory = new Callback<TableColumn<CIS_bdpm, Void>, TableCell<CIS_bdpm, Void>>() {
            ...
            public TableCell<CIS_bdpm, Void> call(final TableColumn<CIS_bdpm, Void> param) {
                final TableCell<CIS_bdpm, Void> cell = new TableCell<CIS_bdpm, Void>() {
                    ...
                    {
                    	conBtn.setOnMousePressed(e -> con.redirectsmr(e,  getTableView().getItems().get(getIndex()).getCode_cis()));
                    }
